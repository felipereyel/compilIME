...
import { Kind, ErrorCode, Obj, Attr, int_, char_, bool_, string_, universal_ } from "./header";
import { Rules, States } from "../syntatical/header";
import Stack from "../stack";

const MAX_NEST_LEVEL = 50;
const IS_TYPE_KIND = (k) => [Kind.ARRAY_TYPE_, Kind.STRUCT_TYPE_, Kind.ALIAS_TYPE_, Kind.SCALAR_TYPE_].includes(k);
  ...
  Error(code) {
    ...
    switch(code) {
      ...
      case ErrorCode.ERR_KIND_NOT_STRUCT:
        error += "Kind not Struct: A operacao so pode ser realizada em tipos Struct";
        break;
      ...
      case ErrorCode.ERR_KIND_NOT_ARRAY:
        error += "Kind not Array: A operacao so pode ser realizada para um Array";
        break;
      ...
      case ErrorCode.ERR_TOO_FEW_ARGS:
        error += "Too Few Args: O numero de parametros especificado nao e suficiente";
        break;
      case ErrorCode.ERR_TOO_MANY_ARGS:
        error += "Too Many Args: O numero de parametros especificado e maior que o especificado";
        break;
      ...
      case ErrorCode.ERR_RETURN_TYPE_MISMATCH:
        error += "Return Type Mismatch: O tipo de retorno não corresponde ao tipo especificado para a função";
        break;
      default:
        break;
    }
  ...
  semantics(rule){
    switch(rule){
      ...
      case Rules.CHR_RULE:
          ...
          this.CHR_.CHR.val = this.lexical.getConst(this.lexical.secondaryToken).value;
      ...
      case Rules.STR_RULE:
          ...
          this.STR_.STR.val = this.lexical.getConst(this.lexical.secondaryToken).value;
      ...
      case Rules.NUM_RULE:
          ...
          this.NUM_.NUM.val = this.lexical.getConst(this.lexical.secondaryToken).value;
      ...
      case Rules.MF_RULE:
          ...
          this.addOut(`BEGIN_FUNC ${this.f.Function.nIndex} ${this.f.Function.nParams} ${this.f.Function.nVars-this.f.Function.nParams}\n`);
          break;
      case Rules.DF_RULE:
          this.endBlock();
      ...
      case Rules.S_RETURN_RULE:
          this.E_ = this.stack.top;
          this.stack.pop();
          if(!this.checkTypes(this.curFunction.Function.pRetType,this.E_.E.type)){
      ...
      case Rules.R_PLUS_RULE:
          ...
          if( !this.checkTypes(this.R1_.R.type,int_) && !this.checkTypes(this.R1_.R.type,string_)){
